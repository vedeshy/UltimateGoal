 ...
 * is explained below.
 */
@TeleOp(name = "Tensor Combo Working", group = "Concept")

public class TensorWorking extends LinearOpMode{
                        ...
                        if (updatedRecognitions != null) {
                            ...
                            if (recognition != null)
                            {
                                ...
                                if (Math.abs(recognition.estimateAngleToObject(AngleUnit.DEGREES)) > 3 && sideways) {
                                    //objectAngle = recognition.estimateAngleToObject(AngleUnit.DEGREES);
                                    leftPower = Range.clip(Math.abs(0.2 * (objectAngle / 20)), 0.1, 0.15);
                                    rightPower = Range.clip(Math.abs(0.2 * (objectAngle / 20)), 0.1, 0.15);
                                    if (objectAngle > 0) {
                                        leftPower *= -1;
                                    } else {
                                        rightPower *= -1;
                                ...
                                if (blockFound && target) {
                                    ...
                                    OdometryHelper.goToPosition(globalPositionUpdate,hardware, this, globalPositionUpdate.returnXCoordinate()+6*COUNTS_PER_INCH,globalPositionUpdate.returnYCoordinate(),0.2,0.5*COUNTS_PER_INCH,0);
                                    ...
                                    OdometryHelper.goToPosition(globalPositionUpdate, hardware, this, globalPositionUpdate.returnXCoordinate(), globalPositionUpdate.returnYCoordinate() + (ratio - 2.5) * COUNTS_PER_INCH, 0.2, 0.5 * COUNTS_PER_INCH, 0);
                                ...
                                if (blockFound) {
                                    OdometryHelper.goToPosition(globalPositionUpdate, hardware, this, 0, 0, 0.3, 1 * COUNTS_PER_INCH, 0);
                                    IMUHelper.gyroTurn(this, hardware,0.0);
                                    break;
                                }
        ...
        while (globalPositionUpdate.returnXCoordinate() < 50*COUNTS_PER_INCH || globalPositionUpdate.returnYCoordinate() < 50*COUNTS_PER_INCH)
        {
            ...
            if (updatedRecognitions!=null)
            {
                ...
                if ((globalPositionUpdate.returnXCoordinate() > 50*COUNTS_PER_INCH || globalPositionUpdate.returnXCoordinate() < 0) && count > 0 && change == false)
                {
                    IMUHelper.gyroTurn(this, hardware,0.0);
                    scalar *= -1;
                    ypos += y_increment*COUNTS_PER_INCH;
                    if (scalar == 1)
                    {
                        xpos = 0*COUNTS_PER_INCH;
                    }
                    else
                    {
                        xpos = 50*COUNTS_PER_INCH;
                    }
                    change = true;
                }
                else
                {
                    ...
                    xpos += scalar*INCREMENT*COUNTS_PER_INCH;
                    if(prevX<0 && xpos < 0)
                    {
                ...
                prevX = xpos;
                OdometryHelper.goToPosition(globalPositionUpdate,hardware,this, xpos, ypos,0.3, 1 * COUNTS_PER_INCH, 0);
